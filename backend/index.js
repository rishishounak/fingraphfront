...
async function ensureTable(retries = 5) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log("✅ Table 'stock_records' is ready");
      return;
    } catch (err) {
      console.error(`❌ Attempt ${i + 1} to create table failed:`, err.code || err);
      await new Promise(res => setTimeout(res, 2000)); // wait 2s before retry
    }
  }
  console.error("🚨 Failed to create table after multiple attempts");
}
...
app.get('/api/stocks', async (req, res) => {
  ...
  if (!ticker) {
    console.warn("⚠️ Missing ticker query parameter");
    return res.status(400).json({ error: "Ticker query parameter required" });
  }

  try {
    console.log(`🔍 Fetching records for ticker: ${ticker.toUpperCase()}`);
    const result = await pool.query(
      `SELECT * FROM stock_records WHERE ticker=$1 ORDER BY record_date DESC LIMIT 10`,
      [ticker.toUpperCase()]
    );
    console.log(`✅ Found ${result.rows.length} records for ${ticker.toUpperCase()}`);
